app.controller('XeditableCtr', ['$scope', '$filter', '$http', 'editableOptions', 'editableThemes',
...
// remove user
        $scope.removeUser = function (index,id, name) {
            var j = confirm("آیا برای حذف آگهی با عنوان " + name + " اطمینان دارید؟ ");
            if (j === true) {
                ...
                $http.post(URL_INSERT, JSON.stringify(datadel))
                    .success(function (result, status, headers, config) {
                        alert("آگهی با کد " + id + " و با عنوان " + name + " با موفقیت حذف شد.");
