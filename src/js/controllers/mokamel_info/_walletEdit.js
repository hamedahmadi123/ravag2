...
app.controller('wallet_edit', ['$scope', '$filter', '$http', function ($scope, $filter, $http, $timeout) {
    $scope.naqdAmount = 0;
    $scope.paytype = 0;
    $scope.passIF = false;
    $scope.sumAmount = 0;
    ...
    function myfunction() {
        ...
        $scope.walid = url_array[url_array.length - 1];
        var stu = {
            ViewName: "SelectWallet",
            parameters: [{key: "%wallet_id", value: $scope.walid}],
            mutualTransaction: {
                kendoDataRequest: {
                    filter: {
                        field: "",
        ...
        $http.post(URL_GET, JSON.stringify(stu))
            .success(function (result, status, headers, config) {
                ...
                $scope.st_id = $scope.wal.student_id;
                $scope.st_name = $scope.wal.firstname + ' ' + $scope.wal.lastname;
                $scope.sal_tahsily = $scope.wal.sal_tahsily;
                $scope.totalAmount = $scope.wal.total_amount;
                $scope.note = $scope.wal.note;
                if ($scope.wal.pay_type == "0") {
                    document.getElementById('naqd').checked = true;
                    $scope.paytype = '0';
                } else {
                    ...
                    $http.post(URL_GET, JSON.stringify(ceq))
                        .success(function (result, status, headers, config) {
                            ...
                            for (var i = 0; i < $scope.chqwal.length; i++) {
                                if ($scope.chqwal[i].type == '0') {
                                    $scope.naqdAmount = $scope.chqwal[i].amount;
                                    $scope.naqdChequeID = $scope.chqwal[i].cheque_id;
                                } else {
                                    $scope.chequeNumber += 1;
                                    $scope.ch_num += 1;
                                    $scope.arrayCheque.push(i);
                                    $scope.obj[i] = {id: i};
                                    $scope.obj[i].cheque_id = $scope.chqwal[i].cheque_id;
                                    $scope.obj[i].amount = $scope.chqwal[i].amount;
                                    $scope.obj[i].date = moment($scope.chqwal[i].cheque_date, 'YYYY-M-D').format('jYYYY-jM-jD');
                                    $scope.obj[i].bank = $scope.chqwal[i].cheque_bank;
                                    $scope.obj[i].shobe = $scope.chqwal[i].cheque_shobe;
                                    $scope.obj[i].cheque_number = $scope.chqwal[i].cheque_number;
                                    $scope.obj[i].owner = $scope.chqwal[i].cheque_owner;
                                }
                            }
    ...
    $scope.updateWallet = function ($id) {
        $scope.eroretext = "";
        $scope.eroretex2 = "";
        var j = confirm(" آیا برای ویرایش این حساب اطمینان دارید ؟");
        if (j === true) {
            if ($scope.sal_tahsily && $scope.totalAmount) {
                ...
                if ($scope.paytype == 0) {
                    var del_chqu1 = {
                        ViewName: "DeleteCheque",
                        parameters: [
                            {key: "%wallet_id", value:$id + ''},
                        ]
                    };
                    $http.post(URL_INSERT, JSON.stringify(del_chqu1))
                        .success(function (result, status, headers, config) {
                    ...
                    $http.post(URL_INSERT, JSON.stringify(ins_wal1))
                        .success(function (result, status, headers, config) {
                            ...
                            document.location.replace("#/app/page/allInWallet");
                        }).error(function (result, status, header, config) {
                    ...
                    if ($scope.naqdAmount) {
                        ...
                        else if (parseInt($scope.totalAmount) > parseInt($scope.sumAmount)) {
                            $scope.passIF = false;
                            ...
                            for (x in $scope.obj) {
                                var ins_smb = {
                                    ...
                                    parameters: [
                                        ...
                                        {key: "%cheque_date", value: moment($scope.obj[x].date, 'jYYYY-jM-jD').format('YYYY-M-D') || '' + ''},
