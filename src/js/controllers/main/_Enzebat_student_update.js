app.controller('Enzebat_student_update', ['$scope', '$filter', '$http', function ($scope, $filter, $http, $timeout) {
    $scope.obj_disType = {};
    ...
    $scope.updateDis = function ($id) {
        if ((localStorage.adminId) && localStorage.adminCount != 0 && (localStorage.adminCount && localStorage.adminCount != undefined && localStorage.adminCount != "undefined")) {
            if (($scope.the_dis_Id) && ($scope.dis_date)) {
                ...
                var j = confirm("آیا برای ویرایش مورد انضباطی برای دانش آموز " + $scope.this_dis.stFName + " "+ $scope.this_dis.stLName + " اطمینان دارید ؟ ");
                if (j === true) {
